...
issue_cf_cert() {
    if is_docker_mode; then
        ...
        require_env "CF_API_KEY"
        require_env "ANYTLS_DOMAIN"
    fi
...
if is_selected 2; then
    ...
    if is_docker_mode; then
        ARGO_PROTOCOL="${ARGO_PROTOCOL:-vless}"
    ...
    if [ "$ARGO_PROTOCOL" != "vless" ] && [ "$ARGO_PROTOCOL" != "vmess" ]; then
        print_msg "ARGO_PROTOCOL 仅支持 vless 或 vmess" red
        exit 1
    fi
    ...
    if is_selected 5; then
        ...
        if is_docker_mode; then
            hy2_port_value="${HY2_PORT1:-20801}"
        else
