...
# ==============================================================================
# 智能兼容版 - All-in-One TUIC & VLESS/VMess+Argo 管理脚本 (v4.0 - 最终完美版)
#
# - 自动检测操作系统 (Alpine, Debian, Ubuntu 等)。
# - 根据系统类型，智能选择安装方式 (apk 或 手动下载)。
# - 采用静态模板生成配置，彻底杜绝逻辑错误。
# - 一个脚本，完美兼容所有主流系统。
# ==============================================================================
...
detect_os_and_set_vars() {
    ...
    if [ -f /etc/alpine-release ]; then
        ...
        OS_TYPE="debian" # Includes Ubuntu
        SINGBOX_CMD="$AGSBX_DIR/sing-box"
        CLOUDFLARED_CMD="$AGSBX_DIR/cloudflared"
    else
        print_msg "错误: 不支持的操作系统。" "red"
        exit 1
    fi
    print_msg "检测到操作系统: $OS_TYPE" "blue"
}
...
do_install() {
    detect_os_and_set_vars # 首先检测系统

    # 1. 安装依赖 (根据系统类型选择方式)
    print_msg "\n--- 正在准备依赖环境 ---" "blue"
    if [ "$OS_TYPE" = "alpine" ]; then
        print_msg "使用 apk 安装依赖..." "yellow"
        apk update
        apk add sing-box cloudflared openssl curl
        if [ $? -ne 0 ]; then print_msg "错误: apk 依赖安装失败。" "red"; exit 1; fi
    else # Debian, Ubuntu, etc.
        print_msg "使用手动下载方式安装依赖..." "yellow"
        local cpu_arch; cpu_arch=$(get_cpu_arch)
        if [ ! -f "$SINGBOX_CMD" ]; then
    ...
    if [ "$INSTALL_CHOICE" = "1" ]; then
        ...
        echo "INSTALL_CHOICE=1" >> "$VARS_PATH"; echo "TUIC_PORT=${TUIC_PORT}" >> "$VARS_PATH"
        ...
        echo "ARGO_TOKEN='${ARGO_TOKEN}'" >> "$VARS_PATH"; echo "ARGO_DOMAIN='${ARGO_DOMAIN}'" >> "$VARS_PATH"
    ...
    # 3. 生成配置
    print_msg "\n--- 正在生成配置文件 ---" "blue"
    local UUID; UUID=$($SINGBOX_CMD generate uuid)
    ...
    if [ "$INSTALL_CHOICE" = "1" ]; then
        cat > "$CONFIG_PATH" <<EOF
{"log":{"level":"info","timestamp":true},"inbounds":[{"type":"tuic","tag":"tuic-in","listen":"::","listen_port":${TUIC_PORT},"users":[{"uuid":"${UUID}","password":"${UUID}"}],"congestion_control":"bbr","tls":{"enabled":true,"server_name":"www.bing.com","alpn":["h3"],"certificate_path":"${CERT_PATH}","key_path":"${KEY_PATH}"}}],"outbounds":[{"type":"direct","tag":"direct"}]}
EOF
    elif [ "$INSTALL_CHOICE" = "2" ]; then
        local argo_inbound=""
        ...
        cat > "$CONFIG_PATH" <<EOF
{"log":{"level":"info","timestamp":true},"inbounds":[${argo_inbound}],"outbounds":[{"type":"direct","tag":"direct"}]}
EOF
    elif [ "$INSTALL_CHOICE" = "3" ]; then
        local argo_inbound=""
        if [ "$ARGO_PROTOCOL" = "vless" ]; then
        ...
        cat > "$CONFIG_PATH" <<EOF
{"log":{"level":"info","timestamp":true},"inbounds":[{"type":"tuic","tag":"tuic-in","listen":"::","listen_port":${TUIC_PORT},"users":[{"uuid":"${UUID}","password":"${UUID}"}],"congestion_control":"bbr","tls":{"enabled":true,"server_name":"www.bing.com","alpn":["h3"],"certificate_path":"${CERT_PATH}","key_path":"${KEY_PATH}"}},${argo_inbound}],"outbounds":[{"type":"direct","tag":"direct"}]}
EOF
    fi
...
do_list() {
    ...
    if [ "$INSTALL_CHOICE" = "2" ] || [ "$INSTALL_CHOICE" = "3" ]; then
        ...
        if [ "$ARGO_PROTOCOL" = "vless" ]; then
            ...
            print_msg "\n--- VLESS + Argo (TLS) 节点 ---" "yellow"; echo "$vless_link"
        else
            ...
            print_msg "\n--- VMess + Argo (TLS) 节点 ---" "yellow"; echo "$vmess_link"
        fi
    fi
}
...
do_start() {
    ...
    nohup $SINGBOX_CMD run -c "$CONFIG_PATH" > "$AGSBX_DIR/sing-box.log" 2>&1 &
...
EOF
            nohup $CLOUDFLARED_CMD tunnel --config "$AGSBX_DIR/config.yml" run --token "$ARGO_TOKEN" > "$AGSBX_DIR/argo.log" 2>&1 &
        else
            nohup $CLOUDFLARED_CMD tunnel --url "http://127.0.0.1:${ARGO_LOCAL_PORT}" > "$AGSBX_DIR/argo.log" 2>&1 &
...
show_help() {
    print_msg "智能兼容版 - All-in-One 管理脚本 (v4.0)" "blue"
    echo "----------------------------------------"
    print_msg "用法: sh $0 [命令]"
    echo
    print_msg "可用命令:" "yellow"
    print_msg "  install    - 安装或重新安装节点"
    print_msg "  list       - 显示已配置的节点信息"
    print_msg "  start      - 启动后台服务"
