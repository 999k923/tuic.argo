...
do_install() {
    ...
    print_msg "  5) 同时安装 AnyTLS + Argo 隧道"
    read -rp "$(printf "${C_GREEN}请输入选项 [1-5]: ${C_NC}")" INSTALL_CHOICE
    ...
    # 下载 cloudflared
    if [[ "$INSTALL_CHOICE" =~ ^(2|3|5)$ ]] && [ ! -f "$CLOUDFLARED_PATH" ]; then
    ...
    # TLS 证书
    if [[ "$INSTALL_CHOICE" = "1" || "$INSTALL_CHOICE" = "3" || "$INSTALL_CHOICE" =~ ^(4|5)$ ]]; then
...
do_list() {
    ...
    if [[ "$INSTALL_CHOICE" =~ ^(4|5)$ ]]; then
        ...
        echo "vless://${UUID}@${server_ip}:${ipv4_port}?encryption=none&security=tls&sni=${ANYTLS_DOMAIN}&alpn=h2,http/1.1&fp=chrome&allow_insecure=1#anytls-ipv4-${hostname}"
        print_msg "--- VLESS + AnyTLS IPv6 ---" yellow
        echo "vless://${UUID}@[${server_ipv6}]:${ipv6_port}?encryption=none&security=tls&sni=${ANYTLS_DOMAIN}&alpn=h2,http/1.1&fp=chrome&allow_insecure=1#anytls-ipv6-${hostname}"
    fi
}
