只保留了TUIC5节点和argo固定隧道（vmess+argo或者vless+argo），2个节点任意安装,

重点：导入节点后IP地址和优选ip地址要手动更换一下，默认的地址都是错误的。
...
重新显示节点信息
如果您忘记了节点链接，或者因为使用临时 Argo 隧道而需要获取最新的域名，可以运行此命令。重点：导入节点后IP地址和优选ip地址要手动更换一下
```bash
./deploy.sh list
```
停止脚本（停止服务）
