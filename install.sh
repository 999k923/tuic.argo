...
# ======================================================================
# All-in-One TUIC & VLESS/VMess+Argo 管理脚本（已改：方案3 自签 rootCA -> leaf，cdn-edge-service.net）
...
ROOTCA_KEY="$AGSBX_DIR/rootCA.key"
ROOTCA_PEM="$AGSBX_DIR/rootCA.pem"
CSR_PATH="$AGSBX_DIR/tuic.csr"
VARS_PATH="$AGSBX_DIR/variables.conf"
# 伪装域名（方案3 固定）
SPOOF_DOMAIN="cdn-edge-service.net"
...
do_install() {
    ...
    # TLS 证书：使用方案3（生成 rootCA -> leaf cert，伪装域名为 cdn-edge-service.net）
    if [[ "$INSTALL_CHOICE" = "1" || "$INSTALL_CHOICE" = "3" ]]; then
        ...
        print_msg "生成 rootCA 与服务证书（伪装域名: ${SPOOF_DOMAIN}）..." yellow

        # 生成 root CA
        openssl genrsa -out "$ROOTCA_KEY" 2048 >/dev/null 2>&1 || { print_msg "生成 rootCA.key 失败" red; exit 1; }
        openssl req -x509 -new -nodes -key "$ROOTCA_KEY" -sha256 -days 36500 \
          -out "$ROOTCA_PEM" \
          -subj "/C=US/ST=VA/L=Ashburn/O=CDN Edge Service/OU=HTTP3 Node/CN=${SPOOF_DOMAIN}" >/dev/null 2>&1 || { print_msg "生成 rootCA.pem 失败" red; exit 1; }

        # 生成 TUIC 私钥
        openssl genrsa -out "$KEY_PATH" 2048 >/dev/null 2>&1 || { print_msg "生成 tuic 私钥失败" red; exit 1; }

        # CSR（leaf cert）
        openssl req -new -key "$KEY_PATH" -out "$CSR_PATH" \
          -subj "/C=US/ST=VA/L=Ashburn/O=CDN Edge Service/OU=HTTP3 Node/CN=${SPOOF_DOMAIN}" >/dev/null 2>&1 || { print_msg "生成 CSR 失败" red; exit 1; }

        # 用 rootCA 签发服务证书
        openssl x509 -req -in "$CSR_PATH" \
          -CA "$ROOTCA_PEM" -CAkey "$ROOTCA_KEY" -CAcreateserial \
          -out "$CERT_PATH" -days 825 -sha256 >/dev/null 2>&1 || { print_msg "签发服务证书失败" red; exit 1; }

        # 权限
        chmod 600 "$KEY_PATH" "$CERT_PATH" "$ROOTCA_KEY" "$ROOTCA_PEM" || true
        print_msg "证书生成完成，rootCA.pem 位于 $ROOTCA_PEM（请导入客户端）" green
    fi
  ...
  "inbounds":[
    {
      ...
      "tls":{
        "enabled":true,
        "server_name":"${SPOOF_DOMAIN}",
        "alpn":["h3","h3-29","h3-30","h2","http/1.1"],
...
do_stop() {
    pkill -f "$SINGBOX_PATH" >/dev/null 2>&1 || true
    pkill -f "$CLOUDFLARED_PATH" >/dev/null 2>&1 || true
...
do_list() {
    ...
    # --- TUIC 节点 ---
    if [[ "$INSTALL_CHOICE" =~ ^(1|3)$ ]]; then
        # URL 中的 alpn 值尽量保留简洁，客户端侧会按配置使用多 ALPN
        alpn_param=$(python3 - <<PY 2>/dev/null
print("h3,h3-29,h3-30,h2,http/1.1")
PY
)
        # url-encode commas and slash
        alpn_enc=$(echo "$alpn_param" | sed 's/,/%2C/g; s/\//%2F/g')
        tuic_params="congestion_control=bbr2&udp_relay_mode=native&alpn=${alpn_enc}&sni=${SPOOF_DOMAIN}"
        ...
        print_msg "\n客户端说明（重要）：" blue
        echo "1) 请把 rootCA 放到客户端并信任： $ROOTCA_PEM"
        echo "2) TUIC 客户端配置示例："
        echo "   server_name = ${SPOOF_DOMAIN}"
        echo "   alpn = [\"h3\",\"h3-29\",\"h2\"]"
        echo "   certificate = rootCA.pem (导入并信任)"
...
show_help() {
    print_msg "All-in-One TUIC & VLESS/VMess+Argo 管理脚本（已伪装: ${SPOOF_DOMAIN}）" blue
    echo "用法: bash $0 [命令]"
